...
export function draftSystemPrompt(catalog: Catalog): string {
  ...
  return [
    ...
    // The SHORTEST branching shape, and by far the most common one people say:
    // check something, then one thing if the result mentions X and another if
    // it does not. The three-automation example below is correct but a poor
    // pattern to copy from — this one is the minimum, and a small model copies
    // structure far better than it follows a rule.
    'Example (branching, the common shape): "check my gmail, if someone emailed me summarize it, if not check again later" = TWO automations. Never one — the CHECK and what happens AFTERWARDS are always separate jobs. {"automations": [{"name": "Gmail check", "category": "Email", "apps": ["gmail"], "steps": ["gmail_recent since_hours=1 limit=10", "answer with how many new emails there are"], "inputs": [], "outputs": [{"name": "found"}], "files": {"reads": [], "writes": []}, "sources": [], "delivers": "answer", "schedule": {"trigger": "watch", "everyMinutes": 5}, ...}, {"name": "Summarize new email", "category": "Email", "apps": ["gmail"], "steps": ["gmail_recent since_hours=1 limit=10", "summarize each new email in one line"], "inputs": [], "outputs": [], "files": {"reads": [], "writes": []}, "sources": [], "delivers": "answer", "schedule": {"trigger": "manual"}, ...}], "chain": {"name": "Gmail check then summarize", "links": [], "steps": [{"id": "s1", "automation": "Gmail check", "after": [], "needs": "all", "when": "ran", "if_answer_contains": null, "if_answer_lacks": null, "map": []}, {"id": "s2", "automation": "Summarize new email", "after": ["s1"], "needs": "all", "when": "ran", "if_answer_contains": "email", "if_answer_lacks": null, "map": []}]}, "question": null}. The "if not" branch needs NO automation when the person only wants it to wait and check again — the watch schedule on s1 IS the checking again.',
