...
import { fetchPersistently } from "./persistentFetch";
...
function systemPrompt(
  ...
  return [
    ...
    "- Simple question → 1-3 sentences of prose. STOP. No lists.",
    '- Multi-part answer → one intro sentence, then 3-7 bullets: "- **Label:** detail", one line each.',
    '- Ranked items (top stories, headlines) → a numbered list, one item per line: "1. **Headline (max 15 words)** — one short clause on why it matters. ([publisher.com](url))". The link goes at the END with the publisher domain as its text — never link the headline itself.',
    "- Copy URLs character-for-character from what you fetched. NEVER invent or shorten a URL.",
    "- Bold only key words and numbers (max 2 per line). Never bold whole sentences. No emoji, no nested lists, no # headings, no ---, no HTML, no raw JSON, no text after the last item.",
    ...
    for (const call of calls) {
      ...
      try {
        if (name === "list_files") {
          ...
          // Blocked ≠ unavailable: this walks the browser-then-mirror ladder
          // before it reports failure, so bot protection on one site doesn't
          // end the run.
          const f = await fetchPersistently(
            String(args.url ?? ""),
            record.sources,
            (text) => onEvent({ text: `Tool loop — ${text}` })
          );
          outcome.logLines.push(f.logLine);
          if (f.ok) outcome.corpus += `\n\n=== ${args.url} ===\n${f.text}`;
          if (!f.ok && f.family === "on_purpose") outcome.refusals.push(f.text);
          result = f.ok
            ? `${f.text}\n---\n${f.logLine}\n${f.substituted ? "That came from a stand-in source — say which one in your answer. " : ""}The job remains: ${record.sentence}`
            : f.text;
